...
namespace CP
{
    ...
    public:
        ...
        void get_at_least(std::vector<T> &v, const T &k)
        {
            recursive(v, k, 0);
        }
        void recursive(std::vector<T> &v, const T &k, int idx)
        {
            if (idx >= mSize)
                return;
            if (mLess(mData[idx], k))
                return;
            v.push_back(mData[idx]);
            recursive(v, k, (idx * 2) + 1);
            recursive(v, k, (idx * 2) + 2);
        }
    };
